...
/**
 * Clase que verifica si existe una declaración de una operación ternaria para contabilizarlo como
 * línea lógica
 */
public class TernaryDeclarationValidator extends LogicalValidator {
  private static final String TERNARY_DECLARATION =
      "(?m)^(\\s*(?:\\w+\\s+\\w+|return)\\s*=?\\s*[^?]+\\?[^:]+:[^;]+;)$";

  /**
   * Determina si una línea de código corresponde a una declaración de una operación ternaria para
   * considerarla como línea lógica
   *
   * @param linesOfCode
   * @return {@code true} si la corresponde a una declaración de una operación ternaria {@code
